"""vy.py — client for the vyakarana (OCaml) Unix socket server.
...
class Client:
    ...
    # ── pipeline helpers ─────────────────────────────────────────────────────
    # These replace the per-file helpers (bqg, answer, refined, run, etc.)
    # that were duplicated across 10+ test files.

    def bqg(self, sentence: str) -> list:
        """Build question graph + avrti-refine to fixpoint. Returns refined graph.

        Replaces the pattern:
            def bqg(vy, s): return vy.eval(f'fixpoint (build-question-graph "{s}") avrti-refine')
        which was duplicated in 10 test files.
        """
        return self.eval(f'fixpoint (build-question-graph "{sentence}") avrti-refine')

    def answer(self, sentence: str) -> str:
        """Run anuvada-ganana end-to-end. Returns the answer text.

        Replaces the pattern:
            def answer(vy, s): return vy.eval(f'anuvada-ganana "{s}"')
        which was duplicated in 8 test files.
        """
        return str(self.eval(f'anuvada-ganana "{sentence}"'))

    def raw_bqg(self, sentence: str) -> list:
        """Build question graph without avrti-refine. Returns raw graph."""
        return self.eval(f'build-question-graph "{sentence}"')

    def full_pipeline(self, sentence: str) -> tuple[list, list]:
        """Run BQG then fixpoint(avrti-refine). Returns (raw_graph, refined_graph).

        Replaces test_pipeline.py::full_pipeline.
        """
        raw = self.raw_bqg(sentence)
        refined = self.eval(f"fixpoint {json.dumps(raw)} avrti-refine")
        return raw, refined

    # ── graph extraction helpers ──────────────────────────────────────────────
    # These replace the inline list comprehensions / dict comprehensions
    # that were repeated across dozens of test files.

    @staticmethod
    def subjects(graph: list, *, pred: str) -> list[str]:
        """Extract subjects where predicate matches.

        Replaces: [t[0] for t in g if t[1] == "satya"]
        """
        return [
            t[0] for t in graph if isinstance(t, list) and len(t) >= 2 and t[1] == pred
        ]

    @staticmethod
    def objects(graph: list, *, pred: str) -> list[str]:
        """Extract objects where predicate matches.

        Replaces: [t[2] for t in g if t[1] == "sankhya"]
        """
        return [
            t[2] for t in graph if isinstance(t, list) and len(t) >= 3 and t[1] == pred
        ]

    @staticmethod
    def triple_map(graph: list, *, pred: str) -> dict[str, str]:
        """Build {subject: object} dict for all triples with given predicate.

        Replaces: {t[0]: t[2] for t in g if t[1] == "sankhya"}
        """
        return {
            t[0]: t[2]
            for t in graph
            if isinstance(t, list) and len(t) >= 3 and t[1] == pred
        }

    @staticmethod
    def sig(graph: list) -> list:
        """Filter out kosha-janya and ppr noise for cleaner assertion messages.

        Replaces the sig() helper duplicated in 8 test files.
        """
        return [
            t
            for t in graph
    ...
    def tl(graph: list) -> str:
        """Serialize a graph to JSON string for use in tantra eval expressions.

        Replaces the tl() helper duplicated in 6 test files.
            vy.eval(f'avrti-refine {vy.tl(g)}')
        """
        return json.dumps(graph)

    @staticmethod
    def strand(answer: str, name: str) -> str:
        """Extract a reasoning strand section from an answer.

        Replaces: r.split("we find")[-1]
        ...
        if len(parts) < 2:
            return ""
        # return text after the strand header, up to the next strand
        tail = parts[-1]
        # cut at next strand boundary
    ...
    @staticmethod
    def strands(answer: str) -> dict[str, str]:
        """Parse all reasoning strands from an answer into a dict.

        Returns: {"we have": "...", "we seek": "...", "we know": "...",
                  "we see": "...", "we find": "..."}
        Only includes strands that are present in the answer.
        """
        strand_names = ["we have", "we seek", "we know", "we see", "we find"]
        result = {}
