"""gates.py — single source of truth for xfail gate classification.

Used by both tests.py (static discovery) and conftest.py (runtime recording).
Each keyword maps to a (gate_name, description) pair.
"""

# keyword -> (gate_name, description)
# first match wins when scanning an xfail reason string
GATE_KEYWORDS = {
    ...
    "colour": (
        "colour_classifier",
        "colour classifiers not treated as entity discriminators",
    ),
    "classifier": (
        "colour_classifier",
        "colour classifiers not treated as entity discriminators",
    ...
    "relative-velocity": (
        "relative_velocity",
        "relative-velocity: kosha concept missing",
    ),
    "compute-then-compare": (
        "compute_compare",
        "viveka: compute-then-compare not built",
    ),
    "compute then compare": (
        "compute_compare",
        "viveka: compute-then-compare not built",
...
def gate_from_reason(reason: str) -> str:
    """Extract gate name from xfail reason string. Returns 'other' on no match."""
    lower = reason.lower()
    for keyword, (gate, _desc) in GATE_KEYWORDS.items():
        if keyword.lower() in lower:
            return gate
    return "other"


def description_from_reason(reason: str) -> str:
    """Extract gate description from xfail reason string."""
    lower = reason.lower()
    for keyword, (_gate, desc) in GATE_KEYWORDS.items():
        if keyword.lower() in lower:
            return desc
    return ""
