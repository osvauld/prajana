...
# vy.py lives in the same directory — ensure importable
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
from vy import Client, DEFAULT_SOCKET

# gate classification — single source of truth
from gates import gate_from_reason, description_from_reason, GATE_KEYWORDS

# fixed cache directory
_TOOLS_DIR = Path(__file__).parent
_FIXED_CACHE = _TOOLS_DIR / ".pytest_cache" / "vyakarana"
...
def pytest_runtest_makereport(item, call):
    ...
    cache_file = _FIXED_CACHE / (_safe_node_id(item.nodeid) + ".json")
