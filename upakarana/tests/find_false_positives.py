...
CACHE_DIR = Path(__file__).parent / ".pytest_cache" / "vyakarana"
TEST_DIR  = Path(__file__).parent

# ─────────────────────────────────────────────────────────────
# Load cache entries (passed tests only)
# ─────────────────────────────────────────────────────────────
...
def load_cache():
    ...
    for f in sorted(CACHE_DIR.glob("*.json")):
        ...
        if e.get("outcome") == "passed":
            test_id = e["test"]   # e.g. "tests/test_answers.py::test_ke_basic"
            # collect all answer/ask call outputs
            outputs = [
                c["output"]
