...
# ── complex: multi-step chains + natural phrasing ────────────────────────────


def test_from_rest_suvat_velocity(vy):
    """from rest + suvat: u=0, a=5, t=3 → v=15"""
    r = vy.answer(
        "a ball of mass 2 accelerates from rest at 5 m/s2 for time 3 seconds. find velocity"
    )
    assert "15" in r


def test_from_rest_force(vy):
    """from rest + F=ma: mass 2000, a=4 → F=8000"""
    r = vy.answer("a truck of mass 2000 accelerates from rest at 4 m/s2. find force")
    assert "8000" in r


def test_centripetal_force_circle(vy):
    """Centripetal: m=2, r=5, v=10 → F=mv²/r=40"""
    r = vy.answer(
        "a ball of mass 2 moves in a circle of radius 5 with velocity 10. find centripetal force"
    )
    assert "40" in r


def test_spring_force_inverse(vy):
    """Inverse spring: F=50, k=250 → x=0.2"""
    r = vy.answer("a spring has spring force 50 and spring constant 250. find displacement")
    assert "0.2" in r


def test_inverse_pressure_natural(vy):
    """Inverse pressure: P=500, F=200 → A=0.4"""
    r = vy.answer("pressure is 500 and force is 200. find area")
    assert "0.4" in r


def test_frequency_from_period(vy):
    """f = 1/T: T=0.02 → f=50"""
    r = vy.answer("a wave has period 0.02. find frequency")
    assert "50" in r


def test_inverse_suvat_find_time(vy):
    """v=50, u=0, a=10 → t=5"""
    r = vy.answer("initial velocity is 0. acceleration is 10. velocity is 50. find time")
    assert "5" in r


def test_at_rest_momentum_zero(vy):
    """at rest → v=0 → p=0"""
    r = vy.answer("a ball of mass 10 is at rest. find momentum")
    assert "0" in r
...
def test_scientific_notation_inverse(vy):
    """Electron KE inverse: KE=4.5e-19, m=9.109e-31 → v≈993999"""
    r = vy.answer("an electron has kinetic energy 4.5e-19 and mass 9.109e-31. find velocity")
    assert "993" in r or "994" in r  # approximate
